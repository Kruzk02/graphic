...
public:
  Mesh(const std::vector<Vertex> &vertices,
       const std::vector<unsigned int> &indices, const VertexLayout &layout)
    ...
    for (const auto &[index, count, type, normalized, offset] :
         layout.attributes) {
  ...
  void draw(const GLenum mode = GL_TRIANGLES) const {
    glBindVertexArray(VAO);
    glDrawElements(mode, static_cast<GLsizei>(indexCount), GL_UNSIGNED_INT,
                   nullptr);
    glBindVertexArray(0);
  }
