...
class Camera {
public:
  explicit Camera(glm::vec3 position, glm::vec3 up,
                  const CameraSettings &settings = {})
      : Camera(position.x, position.y, position.z, up.x, up.y, up.z,
               settings.yaw, settings.pitch, settings) {
    updateCameraVectors();
  }

  Camera(float posX, float posY, float posZ, float upX, float upY, float upZ,
         float yaw, float pitch, const CameraSettings &settings = {})
